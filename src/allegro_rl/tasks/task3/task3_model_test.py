...
parser = argparse.ArgumentParser(description="Evaluate Allegro Hand Task3 TRUE skrl PPO model")
...
class Task3EvalFrameStackWrapper(gym.Env):
    def __init__(self, env: AllegroHandTask3Env, n_stack: int = 5):
        super().__init__()
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        return {"policy": self.obs_stack.clone(), "critic": self.priv_stack.clone()}, info or {}
    ...
    def step(self, actions: torch.Tensor):
        ...
        return {"policy": self.obs_stack.clone(), "critic": self.priv_stack.clone()}, rewards, terminated, truncated, info
...
def resolve_checkpoint(path: str) -> Path:
    ...
    if p.is_dir():
        candidates = [
            p / "allegro_task3_model.pt",
            p / "final_checkpoint" / "allegro_task3_model.pt",
        ]
...
def load_policy_checkpoint(ckpt_path: Path, env: Task3EvalFrameStackWrapper):
...
def force_eval_curriculum(env: AllegroHandTask3Env, start_k: float, label: str) -> None:
    ...
    print(
        ...
        f"global_steps={env.global_steps:,}, phase={env._get_phase()}",
        flush=True,
    )
...
def print_summary_table(summary: Dict[str, Dict[str, float]]) -> None:
    print("\n" + "=" * 184)
    print("Allegro Hand Task3 TRUE skrl PPO Model Test Summary")
    print("=" * 184)
    print(
        f"{'metric':<82} | {'mean':>12} | {'std':>12} | {'min':>12} | "
...
def main() -> None:
    ...
    base_env = AllegroHandTask3Env(cfg)
    force_eval_curriculum(base_env, args_cli.start_k, "after_env_creation")

    env = Task3EvalFrameStackWrapper(base_env, n_stack=int(args_cli.frame_stack))
    ...
    print("\n" + "=" * 150)
    print("Allegro Hand Task3 TRUE skrl PPO model test started")
    print("=" * 150)
    ...
    print(f"phase              : {base_env._get_phase()}")
    ...
    print("actor/critic       : asymmetric, actor obs + privileged critic obs")
    ...
    try:
        with tqdm(
            ...
            desc="Allegro Task3 skrl Model Test",
            dynamic_ncols=True,
            mininterval=0.5,
        ) as pbar:
                ...
                if step % max(int(args_cli.print_interval), 1) == 0 or step == int(args_cli.steps) - 1:
                    ...
                    pbar.set_postfix(
                        {
                            ...
                            "objh": f"{flat.get('telemetry/Obj_H', 0.0):.3f}",
                    ...
                    if bool(args_cli.visualize):
                        sys.stdout.write(
                            ...
                            f"Phase={flat.get('telemetry/Phase', 0.0):.1f} | "
                            f"TCP={flat.get('telemetry/TCP_Dist', 0.0):.3f} | "
                            f"Pre={flat.get('telemetry/TCP_Pregrasp_Dist', 0.0):.3f} | "
                            f"Soft={flat.get('telemetry/SoftContact_Count', 0.0):.2f} | "
                            f"Hard={flat.get('telemetry/HardContact_Count', flat.get('telemetry/Contact_Count', 0.0)):.2f} | "
                            f"Force={flat.get('telemetry/Force_Max', 0.0):.4f} | "
                            f"Lift={flat.get('telemetry/Lift', 0.0):.3f} | "
                            f"BaseH={flat.get('telemetry/Base_H', 0.0):.3f} | "
                            f"R={row['test/reward_mean']:+.3f} | "
                            f"Drop={flat.get('events/Drop', 0.0):.2f} | "
                            f"Slide={flat.get('events/SlideOut', 0.0):.2f} | "
                            f"Crash={flat.get('events/TableCrash', 0.0):.2f}"
                        )
                        sys.stdout.flush()
        ...
        print("\n✅ Allegro Task3 TRUE skrl PPO model test rollout finished")
        ...
        print("Allegro Task3 model test checklist:")
        print("1. checkpoint metadata 必须标记 uses_skrl=True。")
        print("2. Task3 必须是 asymmetric actor-critic checkpoint。")
        print("3. 默认 start_k=1.0，测试最终课程阶段。")
        print("4. 测试脚本不调用 agent.act，避免模型测试卡在 0%。")
        print("5. smoke checkpoint 效果差是正常的，先看推理稳定性和无 NaN/Inf。")
        print("6. 正式效果重点看 TCP_Dist、Contact_Count、Lift、Obj_H、Drop、SlideOut、TableCrash、SO3_Err。")

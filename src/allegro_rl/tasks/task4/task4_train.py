...
parser = argparse.ArgumentParser(description="Train Allegro Hand Task4 Blind Sim2Real/RMA Teacher with TRUE skrl PPO")
...
def make_run_name() -> str:
    ...
    if run_name:
        return run_name
    return f"allegro_task4_skrl_teacher_ppo_{datetime.now().strftime('%Y%m%d_%H%M%S')}"


class Task4TeacherFrameStackAndLogWrapper(gym.Env):
    """Task4 Teacher PPO wrapper.

    Actor:
        teacher_obs * frame_stack

    Critic:
        privileged_obs * frame_stack

    Stored for later RMA / student:
        blind obs
        history obs
    """

    def __init__(self, env: AllegroHandTask4Env, log_dir: str, n_stack: int = 5):
        super().__init__()
        ...
        self.last_blind_obs = torch.zeros((self.num_envs, self.blind_dim), dtype=torch.float32, device=self.device)
        self.last_history_obs = torch.zeros((self.num_envs, self.history_dim), dtype=torch.float32, device=self.device)
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None, **kwargs):
        ...
        return {"policy": self.policy_stack.clone(), "critic": self.critic_stack.clone()}, self.last_info
    ...
    def step(self, actions: torch.Tensor):
        ...
        try:
            ...
            self.writer.add_scalar("rma/blind_obs_norm", torch.norm(self.last_blind_obs, dim=-1).mean().item(), self.global_env_steps)
            self.writer.add_scalar("rma/history_obs_norm", torch.norm(self.last_history_obs, dim=-1).mean().item(), self.global_env_steps)
        except Exception:
            pass

        return {"policy": self.policy_stack.clone(), "critic": self.critic_stack.clone()}, rewards, terminated, truncated, self.last_info
...
class Task4PrivilegedCritic(DeterministicMixin, Model):
    def __init__(self, observation_space, state_space, action_space, device):
        ...
        if state_space is None:
            raise RuntimeError("env.state_space is None. Task4 critic requires privileged state space.")
        ...
        self.apply(Task4TeacherActor._init_weights)
...
def resolve_resume_checkpoint(path: str) -> str:
    ...
    if p.is_dir():
        candidates = [
            p / "allegro_task4_skrl_agent.pt",
            p / "teacher_model.pt",
            p / "agent.pt",
            p / "final_checkpoint" / "allegro_task4_skrl_agent.pt",
            p / "final_checkpoint" / "teacher_model.pt",
...
def save_project_checkpoint(
    ...
    env_cfg: Task4Config,
    env: Task4TeacherFrameStackAndLogWrapper,
    env_steps: int,
    args,
) -> None:
    os.makedirs(directory, exist_ok=True)

    skrl_agent_path = os.path.join(directory, "allegro_task4_skrl_agent.pt")
    eval_model_path = os.path.join(directory, "allegro_task4_teacher_model.pt")

    try:
        agent.save(skrl_agent_path)
        agent.save(os.path.join(directory, "teacher_model.pt"))
    ...
    torch.save(
        {
            ...
            "metadata": {
                "robot": "Allegro Hand",
                "task": "task4_blind_sim2real_rma",
                "stage": "task4_teacher_ppo",
                "algorithm": "skrl_PPO",
                "uses_skrl": True,
                "asymmetric_actor_critic": True,
                "teacher_policy": True,
                "student_policy": False,
                "rma_ready": True,
                "policy_input": "teacher_obs_stack_5",
                "critic_input": "privileged_obs_stack_5",
                "blind_obs_available": True,
                "history_obs_available": True,
                "blind_obs_dim": int(env_cfg.num_observations),
                "teacher_single_obs_dim": int(env_cfg.num_teacher_obs),
                "privileged_single_obs_dim": int(env_cfg.num_privileged_obs),
                "history_obs_dim": int(env_cfg.history_obs_dim),
                "history_frame_dim": int(env_cfg.history_frame_dim),
                "history_length": int(env_cfg.history_length),
                "frame_stack": int(args.frame_stack),
                "actor_obs_dim": int(env_cfg.num_teacher_obs * int(args.frame_stack)),
                "critic_obs_dim": int(env_cfg.num_privileged_obs * int(args.frame_stack)),
                "action_dim": int(env_cfg.num_actions),
                "note": "TRUE skrl PPO teacher checkpoint. Evaluation uses deterministic policy forward, not agent.act.",
            },
        },
        eval_model_path,
    )
    ...
    torch.save(
        {
            ...
            "env_cfg": {
                ...
                "num_teacher_obs": int(env_cfg.num_teacher_obs),
                "num_privileged_obs": int(env_cfg.num_privileged_obs),
                "history_obs_dim": int(env_cfg.history_obs_dim),
                "history_length": int(env_cfg.history_length),
                "history_frame_dim": int(env_cfg.history_frame_dim),
        ...
        os.path.join(directory, "task4_train_metadata.pt"),
    )

    print(f"💾 [Allegro Task4 Teacher skrl checkpoint] saved to: {directory}", flush=True)
...
def main() -> None:
    ...
    print("\n" + "=" * 118)
    print("🚀 Allegro Hand Task4 Blind Sim2Real / RMA Teacher - TRUE skrl PPO Training")
    print("=" * 118)
    print(f"[INFO] PROJECT_ROOT = {PROJECT_ROOT}")
    print(f"[INFO] log_root     = {log_dir}")
    print(f"[INFO] run_name     = {run_name}")
    print("[INFO] This version uses skrl PPO with teacher_obs actor and privileged_obs critic.")
    ...
    base_env = AllegroHandTask4Env(env_cfg)
    ...
    print("\n[INFO] Task4 automatic curriculum")
    print(f"  - K               : {base_env._k():.4f}")
    print(f"  - DR_K            : {base_env._dr_k():.4f}")
    print(f"  - Reward_K        : {base_env._rw_k():.4f}")
    print(f"  - DR schedule     : {env_cfg.dr_start_k:.2f} -> {env_cfg.dr_end_k:.2f}")
    print(f"  - Reward schedule : {env_cfg.reward_start_k:.2f} -> {env_cfg.reward_end_k:.2f}")

    run_dir = os.path.join(log_dir, run_name)
    local_env = Task4TeacherFrameStackAndLogWrapper(base_env, log_dir=run_dir, n_stack=int(args_cli.frame_stack))

    env = wrap_env(local_env, wrapper="isaaclab")
    num_envs = getattr(env, "num_envs", local_env.num_envs)

    if env.state_space is None:
        raise RuntimeError("env.state_space is None. Task4 Teacher requires privileged critic state space.")

    print("\n[DEBUG] Allegro Task4 Teacher Spaces")
    print(f"  env.observation_space = {env.observation_space}")
    print(f"  env.state_space       = {env.state_space}")
    print(f"  env.action_space      = {env.action_space}")
    print(f"  teacher single dim    = {local_env.teacher_dim}")
    print(f"  privileged single dim = {local_env.priv_dim}")
    print(f"  blind single dim      = {local_env.blind_dim}")
    print(f"  history dim           = {local_env.history_dim}")
    ...
    print(f"  - teacher_single_obs  : {local_env.teacher_dim}")
    print(f"  - privileged_single   : {local_env.priv_dim}")
    print(f"  - blind_single_obs    : {local_env.blind_dim}")
    print(f"  - history_obs_dim     : {local_env.history_dim}")
    ...
    print("\n🔥 [Allegro Task4 Teacher TRUE skrl PPO 已点火]")
    print("👉 Actor 输入: teacher_obs × 5 帧")
    print("👉 Critic 输入: privileged_obs × 5 帧")
    print("👉 Blind obs / history_obs 同步记录，后续用于 RMA Adapter / Student 蒸馏")
    print("👉 自动课程: Warmup DR -> Full DR，无需手动改代码\n")
    ...
    try:
        ...
        with tqdm(
            ...
            desc="Allegro Task4 Teacher skrl PPO",
            unit="steps",
            dynamic_ncols=True,
            mininterval=0.5,
            smoothing=0.05,
            ...
            for t in range(total_vector_steps):
                ...
                pbar.set_postfix(
                    {
                        ...
                        "RW": f"{flat.get('telemetry/Reward_K', 0.0):.2f}",
                    ...
                    if update_id % max(int(args_cli.summary_interval), 1) == 0:
                        ...
                        pbar.write(
                            "\n".join(
                                [
                                    "\n" + "=" * 118,
                                    f"📊 [Allegro Task4 Teacher skrl PPO 更新 {update_id}] "
                ...
                if env_steps - last_save >= save_freq_env_steps:
                    ...
                    try:
                        ...
                        pbar.write(f"\n💾 [Allegro Task4 Teacher skrl 备份] 总步数: {env_steps:,} | 已保存至: {save_dir}\n")
                    except Exception as exc:
                        pbar.write(f"\n[WARN] checkpoint 保存失败: {type(exc).__name__}: {exc}\n")

    except KeyboardInterrupt:
        print("\n[WARN] 接收到 Ctrl+C，正在保存当前 Task4 Teacher skrl 模型...")
    except Exception:
        print("\n[ERROR] Allegro Task4 Teacher skrl PPO 训练过程中发生真实异常：")
        traceback.print_exc()
        raise
    finally:
        ...
        try:
            ...
            print(f"✅ Allegro Task4 Teacher skrl 模型已保存至 {final_dir}")
        except Exception as exc:
        ...
        print("✅ Allegro Task4 Teacher TRUE skrl PPO training pipeline safely exited")

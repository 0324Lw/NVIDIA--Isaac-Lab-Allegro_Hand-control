...
parser = argparse.ArgumentParser(description="Evaluate Allegro Hand Task2 TRUE skrl PPO model")
...
class Task2EvalFrameStackWrapper(gym.Env):
    def __init__(self, env: AllegroHandTask2Env, n_stack: int = 5):
        super().__init__()
...
def resolve_checkpoint(path: str) -> Path:
    ...
    if p.is_dir():
        candidates = [
            p / "allegro_task2_model.pt",
            p / "final_checkpoint" / "allegro_task2_model.pt",
        ]
...
def load_policy_checkpoint(ckpt_path: Path, env: Task2EvalFrameStackWrapper):
    ...
    if not bool(metadata.get("asymmetric_actor_critic", False)):
        raise RuntimeError(
            ...
            "Task2 需要 actor obs / privileged critic obs 非对称结构。"
        )
...
def force_eval_curriculum(env: AllegroHandTask2Env, start_k: float, label: str) -> None:
...
def print_summary_table(summary: Dict[str, Dict[str, float]]) -> None:
    print("\n" + "=" * 176)
    print("Allegro Hand Task2 TRUE skrl PPO Model Test Summary")
    print("=" * 176)
    print(
...
def main() -> None:
    ...
    base_env = AllegroHandTask2Env(cfg)
    force_eval_curriculum(base_env, args_cli.start_k, "after_env_creation")

    env = Task2EvalFrameStackWrapper(base_env, n_stack=int(args_cli.frame_stack))
    ...
    print("\n" + "=" * 150)
    print("Allegro Hand Task2 TRUE skrl PPO model test started")
    print("=" * 150)
    ...
    try:
        with tqdm(
            ...
            desc="Allegro Task2 skrl Model Test",
            dynamic_ncols=True,
            mininterval=0.5,
        ) as pbar:
                ...
                if step % max(int(args_cli.print_interval), 1) == 0 or step == int(args_cli.steps) - 1:
                    ...
                    pbar.set_postfix(
                        {
                            ...
                            "drop": f"{flat.get('events/Drop_Rate', 0.0):.2f}",
                    ...
                    if bool(args_cli.visualize):
                        sys.stdout.write(
                            ...
                            f"Err={flat.get('telemetry/Geodesic_Error_Rad', 0.0):.4f} rad | "
                            f"Height={flat.get('telemetry/Object_Height', 0.0):.4f} m | "
                            f"Contacts={flat.get('telemetry/Active_Contacts', 0.0):.2f} | "
        ...
        print("\n✅ Allegro Task2 TRUE skrl PPO model test rollout finished")
        ...
        print("Allegro Task2 model test checklist:")
        print("1. checkpoint metadata 必须标记 uses_skrl=True。")
        print("2. Task2 必须是 asymmetric actor-critic checkpoint。")
        print("3. 默认 start_k=1.0，测试最终姿态重定向难度。")
        print("4. 测试脚本不调用 agent.act，避免模型测试卡在 0%。")
        print("5. smoke checkpoint 效果差是正常的，先看推理稳定性和无 NaN/Inf。")
        print("6. 正式效果重点看 Object_Height、Drop_Rate、Active_Contacts、Geodesic_Error_Rad。")

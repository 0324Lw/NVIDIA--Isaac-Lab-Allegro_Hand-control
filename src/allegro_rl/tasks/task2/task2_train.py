...
parser = argparse.ArgumentParser(description="Train Allegro Hand Task2 in-hand reorientation with TRUE skrl PPO")
...
parser.add_argument("--value-coef", type=float, default=2.5)
...
def make_run_name() -> str:
    ...
    if run_name:
        return run_name
    return f"allegro_task2_skrl_ppo_{datetime.now().strftime('%Y%m%d_%H%M%S')}"


class Task2DictFrameStackAndLogWrapper(gym.Env):
    """Task2 asymmetric observation wrapper.

    Actor:
        stacked actor obs = 83 * frame_stack

    Critic:
        stacked privileged obs = 88 * frame_stack
    """

    def __init__(self, env: AllegroHandTask2Env, log_dir: str, n_stack: int = 5):
        super().__init__()
...
class AsymmetricCritic(DeterministicMixin, Model):
    def __init__(self, observation_space, state_space, action_space, device):
        ...
        if state_space is None:
            raise RuntimeError("env.state_space is None. Task2 critic requires privileged state space.")
...
def resolve_resume_checkpoint(path: str) -> str:
    ...
    if p.is_dir():
        candidates = [
            p / "allegro_task2_skrl_agent.pt",
            p / "agent.pt",
            p / "final_checkpoint" / "allegro_task2_skrl_agent.pt",
...
def save_project_checkpoint(
    ...
    env_cfg: Task2Config,
    env: Task2DictFrameStackAndLogWrapper,
    env_steps: int,
    args,
) -> None:
    os.makedirs(directory, exist_ok=True)

    skrl_agent_path = os.path.join(directory, "allegro_task2_skrl_agent.pt")
    eval_model_path = os.path.join(directory, "allegro_task2_model.pt")
    ...
    torch.save(
        {
            ...
            "metadata": {
                "robot": "Allegro Hand",
                "task": "task2_inhand_reorientation",
                "algorithm": "skrl_PPO",
                "uses_skrl": True,
    ...
    print(f"💾 [Allegro Task2 skrl checkpoint] saved to: {directory}", flush=True)
...
def main() -> None:
    ...
    print("\n" + "=" * 118)
    print("🚀 Allegro Hand Task2 In-Hand Reorientation - TRUE skrl PPO Training")
    print("=" * 118)
    ...
    base_env = AllegroHandTask2Env(env_cfg)
    ...
    local_env = Task2DictFrameStackAndLogWrapper(base_env, log_dir=run_dir, n_stack=int(args_cli.frame_stack))

    env = wrap_env(local_env, wrapper="isaaclab")
    num_envs = getattr(env, "num_envs", local_env.num_envs)

    if env.state_space is None:
        raise RuntimeError("env.state_space is None. Task2 requires privileged critic state space.")

    print("\n[DEBUG] Allegro Task2 Spaces")
    ...
    print("\n🔥 [Allegro Task2 TRUE skrl PPO 已点火]\n")
    ...
    try:
            ...
            for t in range(total_vector_steps):
                ...
                pbar.set_postfix(
                    {
                        ...
                        "contact": f"{flat.get('telemetry/Active_Contacts', 0.0):.2f}",
                        "err": f"{flat.get('telemetry/Geodesic_Error_Rad', 0.0):.3f}",
                    ...
                    if update_id % max(int(args_cli.summary_interval), 1) == 0:
                        ...
                        pbar.write(
                            "\n".join(
                                [
                                    "\n" + "=" * 118,
                                    f"📊 [Allegro Task2 skrl PPO 更新 {update_id}] "
                ...
                if env_steps - last_save >= int(args_cli.save_freq_env_steps):
                    ...
                    try:
                        ...
                        pbar.write(f"\n💾 [Allegro Task2 skrl 备份] 总步数: {env_steps:,} | 已保存至: {save_dir}\n")
                    except Exception as exc:
        ...
        print("\n[ERROR] Allegro Task2 skrl PPO 训练过程中发生真实异常：")
        traceback.print_exc()
        raise
    finally:
        ...
        try:
            ...
            print(f"✅ Allegro Task2 skrl 模型已保存至 {final_dir}")
        except Exception as exc:
        ...
        print("✅ Allegro Task2 TRUE skrl PPO training pipeline safely exited")
